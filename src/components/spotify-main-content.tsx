...
// Export data arrays for use in database-agent.ts
...
export default function SpotifyMainContent({ onPlayTrack }: SpotifyMainContentProps) {
  // State for API data with fallback to hardcoded data
  const [apiData, setApiData] = useState({
    recentlyPlayed: recentlyPlayedData,
    madeForYou: madeForYouData,
    popularAlbums: popularAlbumsData
  })
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        // Keep hardcoded data as fallback on error
      }
    }
  ...
  // Use API data (with hardcoded fallback)
  const recentlyPlayed = apiData.recentlyPlayed
  const madeForYou = apiData.madeForYou
  const popularAlbums = apiData.popularAlbums
